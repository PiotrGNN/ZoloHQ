...
def test_pandas(ford_2012):
    try:
        if talib is None or ford_2012 is None:
            pytest.skip("talib or fixture ford_2012 not available; skipping test.")
        import pandas
...
def test_SMA(ford_2012):
    try:
        ...
        assert_array_not_equal(
            expected, abstract.Function("sma", ford_2012)(10, price="open")
        )
    except (NameError, UnboundLocalError, ImportError) as e:
        pytest.skip(f"Dependency or symbol missing: {e}")
...
def test_MAVP(ford_2012):
    ...
    except (NameError, UnboundLocalError, ImportError, Exception) as e:
        pytest.skip(f"Dependency or symbol missing or error: {e}")
        return
...
def test_input_arrays(ford_2012):
    try:
        ...
        try:
            ...
            pytest.skip("Expected ValueError not raised; skipping test.")
            return
...
def skip_if_talib_missing():
    if 'talib' not in globals() or talib is None:
        pytest.skip("talib not available; skipping all TA-Lib abstract tests.")
