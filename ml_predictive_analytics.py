...
class MLPredictiveAnalytics:
    def __init__(self):
        ...
        try:
            ...
            self.logger.warning(
                "Production data manager not available - using API fallback"
            )
    ...
    def fetch_real_trading_data(self):
        ...
        try:
            ...
            if st:
                st.error(f"Failed to fetch real trading data: {e}")
            return self.generate_synthetic_data()
    ...
    def predict_future_performance(self, current_data, days_ahead=7):
        ...
        try:
            ...
            if st:
                st.error(f"Error predicting future performance: {e}")
            return None
    ...
    def generate_ml_insights(self, data, historical_df):
        ...
        try:
            ...
            if SKLEARN_AVAILABLE and len(historical_df) > 10:
                ...
                X = historical_df[feature_cols].fillna(
                    historical_df[feature_cols].mean()
                )
                ...
                best_cluster = max(
                    cluster_stats.keys(), key=lambda x: cluster_stats[x]["avg_profit"]
                )
            ...
            # Risk analysis
            current_risk = np.mean(
                [float(bot.get("risk_score", 50)) for bot in data.get("bots", [])]
            )
            if current_risk > 70:
                insights.append(
                    {
    ...
    def get_real_historical_data(self, symbol="BTCUSDT", interval="1h", limit=1000):
        ...
        if self.production_manager and self.production_mode:
            try:
                ...
                historical_data = self.production_manager.get_historical_data(
                ...
                if not historical_data.empty:
                    ...
                    self.logger.info(
                        f"Retrieved {len(ml_data)} real data points for ML training"
                    )
                    return ml_data
            except Exception as e:
    ...
    def _transform_market_data_to_ml_format(self, market_df):
        ...
        try:
            ...
            for _i, row in market_df.iterrows():
                ...
                win_rate = max(
                    30, min(90, 65 + daily_return * 2)
                )  # Base win rate with market bias
                ...
                ml_data.append(
                    {
                        ...
                        "market_sentiment": (
                            "bullish"
                            if daily_return > 1
                            else "bearish" if daily_return < -1 else "neutral"
                        ),
...
def main():
    ...
    st.markdown(
        '<div class="main-header">🤖 ML Predictive Analytics Dashboard</div>',
        unsafe_allow_html=True,
    )
    ...
    if not SKLEARN_AVAILABLE:
        st.sidebar.warning(
            "Install scikit-learn for full ML features:\n`pip install scikit-learn`"
        )
    ...
    if analytics.production_mode:
        try:
            ...
            if historical_df.empty:
                ...
                st.info(
                    f"📡 Using {len(historical_df)} real data points for ML training"
                )
        except Exception as e:
    ...
    if train_models and SKLEARN_AVAILABLE:
        ...
        with col1:
            if (
                analytics.production_mode
                and current_data.get("data_source") == "production_manager"
            ):
                # Real portfolio data
                balance_data = current_data.get("balance", {}).get("result", {})
                float(balance_data.get("totalWalletBalance", 0))
        ...
        if "profit_predictor" in analytics.models:
            ...
            if prediction:
                ...
                with col1:
                    # Prediction chart
                    dates = pd.date_range(
                        start=datetime.now(), periods=prediction_days + 1, freq="D"
                    )
                    current_profit = total_profit

                    # Create prediction line
                    profit_progression = np.linspace(
                        current_profit,
                        prediction["predicted_profit"],
                        prediction_days + 1,
                    )
                    confidence_upper = np.linspace(
                        current_profit,
                        prediction["confidence_upper"],
                        prediction_days + 1,
                    )
                    confidence_lower = np.linspace(
                        current_profit,
                ...
                with col2:
                    ...
                    change_percent = (
                        (predicted_change / current_profit) * 100
                        if current_profit != 0
                        else 0
                    )
                    ...
                    confidence_score = max(
                        ...
                        min(
                            100,
                            100
                            - (confidence_range / prediction["predicted_profit"] * 100),
                        ),
                    )
    ...
    with tabs[2]:
        ...
        if analytics.anomaly_detector:
            ...
            with col1:
                if anomalies:
                    ...
                    for i, anomaly in enumerate(anomalies):
                        severity_color = (
                            "#e74c3c" if anomaly["severity"] == "High" else "#f39c12"
                        )
                        st.markdown(
                            f"""
            ...
            with col2:
                ...
                st.slider("Sensitivity", 0.01, 0.3, 0.1, 0.01)
    ...
    with tabs[3]:
        ...
        if insights:
            for insight in insights:
                icon = {"clustering": "🔍", "trend": "📈", "risk": "⚠️"}.get(
                    insight["type"], "💡"
                )
        ...
        with col1:
            ...
            if len(historical_df) > 10:  # Check which columns are available
                ...
                available_cols = [
                    col for col in required_cols if col in historical_df.columns
                ]
                ...
                st.warning(
                    "📊 Insufficient data for correlation analysis (need >10 records)"
                )
    ...
    with tabs[4]:
        ...
        with col2:
            ...
            training_dates = pd.date_range(
                start=datetime.now() - timedelta(days=30), periods=10, freq="3D"
            )
